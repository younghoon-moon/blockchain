...
이더리움 블록체인은 비트코인 블록체인과 마찬가지로 블록의 체인 형태로 구성되어 있는 데이터베이스이다. 

#### 3.1. 이더리움 가상머신\(EVM: Ethereum Virtual Machine\)

이더리움 가상머신\(EVM\)은 누구나 EVM 바이트코드\(bytecode\)를 실행시킬 수 있도록 튜링 완전한\(Turing-complete\) 256비트의 가상머신이다. 솔리디티\(Solidity\)와 같은 스마트 컨트랙트 언어
