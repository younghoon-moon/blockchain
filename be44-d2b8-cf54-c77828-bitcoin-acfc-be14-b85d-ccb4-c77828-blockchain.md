...
비트코인은 누구나 비트코인 네트워크에 참가하여 **아무런 허가 없이** 1. 비트코인을 채굴하거나 2. 비트코인으로 이루어지는 거래의 유효성을 검증하거나 3. 비트코인 계좌를 만들고 다른 참가자와 자유롭게 거래 할 수 있는 비허가성\(permission-less\) 시스템이다.
...
이는 현재의 금융 시스템에서 계좌를 만들기 위해 자신의 신원 및 개인정보를 증명하는 복잡한 절차를 거쳐야 하는 것과는 대조적이다.
